...
class VideoQADataset(Dataset):
    def __init__(
        ...
        '''
        if self.dset not in ['webvid', 'frameqa', 'star', 'causalvid']:  # object detection
            filen = bnum
            ...
            with h5py.File(bbox_feat_file, 'r') as fp:
                vids = fp['ids']
                feats = fp['feat'] 
                print(feats.shape) #v_num, clip_num, region_per_frame, bbox, feat_dim (3870, 8, 4, 20, 2048)
                bboxes = fp['bbox'] #(3870, 8, 4, 20,4)
                for id, (vid, feat, bbox) in enumerate(zip(vids, feats, bboxes)): 
                    #(clip, frame, bbox, feat), (clip, frame, bbox, coord)           # 각 feature들의 정확한 의미가 뭔지
                    if self.dset == 'star': vid = vid.decode("utf-8")
                    self.bbox_feats[str(vid)] = (feat[:,:,:self.bbox_num, :], bbox[:,:,:self.bbox_num, :]) # (8, 4, 5, 2048) (8, 4, 5, 4)
        '''
        
        '''    
        if self.dset not in ['causalvid','webvid', 'star']:  # frame appearance feature
            if self.use_frame:
                ...
                with h5py.File(app_feat_file, 'r') as fp:
                    ...
                    print(feats.shape) #v_num, clip_num, length, feat_dim
    ...
    def __getitem__(self, index):
        ...
        if self.lvq and self.mode not in ['val','test']:
            ...
            qtype = get_qsn_type(question_txt, qtype) # 현재 문제의 질문 type
            neg_num = 5
        ...
        if self.mc:
            ...
            if self.dset=='webvid': # and self.mode == 'train':
                ...
                answer_id = choices.index(ans) if ans in choices else -1          # 정답의 인덱스 

                if self.mode not in ['train','val', 'test'] and rd.random() < 0.3: # 30%의 확률로 .. 이 확률을 바꿔볼까 또는 없애볼까
                ...
                answer_txts = [question_txt+f' {self.tokenizer.sep_token} '+opt for opt in choices] # question + </s> + answer_cand 왜 이렇게 ?
